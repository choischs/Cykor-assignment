...
void func1(int arg1, int arg2, int arg3)
{
	...
	call_stack[0] = 3;       // 매개변수 arg1, arg2, arg3을 call_stack에 push
	...
	call_stack[3] = -1;      // 반환 주소값을 call_stack에 push
	snprintf(stack_info[3], sizeof(stack_info[3]), "Return Address");
	call_stack[4] = -1;      // SFP를 call_stack에 push
	snprintf(stack_info[4], sizeof(stack_info[4]), "func1 SFP");
	call_stack[5] = var_1;   // 지역변수 var_1을 call_stack에 push
	snprintf(stack_info[5], sizeof(stack_info[5]), "var_1");
	SP = 5; FP = 4;          // SP와 FP 업데이트

	print_stack();
	func2(11, 13);
	// func2의 스택 프레임 제거 (함수 에필로그 + pop)
	for (int j = SP; j > FP; j--) {      // FP까지 지역변수 pop
		call_stack[j] = -1;
		snprintf(stack_info[j], sizeof(stack_info[j]), "");
	}
	SP = FP; FP = call_stack[FP];        // SP와 FP 업데이트
	call_stack[SP] = -1;               // SFP를 call_stack에 pop
	snprintf(stack_info[SP], sizeof(stack_info[SP]), "");
	call_stack[SP - 1] = -1;           // 반환 주소값을 call_stack에 pop
	snprintf(stack_info[SP - 1], sizeof(stack_info[SP - 1]), "");
	SP = SP - 2;                         // SP 업데이트
	call_stack[SP] = -1;               // 매개변수 arg1, arg2을 call_stack에 pop
	snprintf(stack_info[SP], sizeof(stack_info[SP]), "");
	SP = SP - 1;
	call_stack[SP] = -1;
	snprintf(stack_info[SP], sizeof(stack_info[SP]), "");
...
void func2(int arg1, int arg2)
{
	...
	snprintf(stack_info[SP - 1], sizeof(stack_info[SP - 1]), "");
	SP = SP - 2;                         // SP 업데이트
	call_stack[SP] = -1;               // 매개변수 arg1을 call_stack에 pop
	snprintf(stack_info[SP], sizeof(stack_info[SP]), "");
	SP = SP - 1;                         // SP 업데이트
...
int main()
{
	...
	for (int j = SP; j > FP; j--) {      // FP까지 pop
		call_stack[j] = -1;
		snprintf(stack_info[j], sizeof(stack_info[j]), "");
	}
	SP = FP; FP = call_stack[FP];        // SP와 FP 업데이트
	call_stack[FP] = -1;               // SFP를 call_stack에 pop
	snprintf(stack_info[FP], sizeof(stack_info[FP]), "");
	call_stack[FP - 1] = -1;           // 반환 주소값을 call_stack에 pop
	snprintf(stack_info[FP - 1], sizeof(stack_info[FP - 1]), "");
	SP = SP - 2;                         // SP 업데이트
	call_stack[SP] = -1;               // 매개변수 arg1, arg2, arf3을 call_stack에 pop
	snprintf(stack_info[SP], sizeof(stack_info[SP]), "");
	SP = SP - 1;
	call_stack[SP] = -1;
	snprintf(stack_info[SP], sizeof(stack_info[SP]), "");
